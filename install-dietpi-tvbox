#!/bin/bash
{
    ...
    ### FORCE NO KERNEL HANDLING (TV BOX PATCH)
    export DIETPI_SKIP_KERNEL=1
    export DIETPI_DISABLE_KERNEL_INSTALL=1
    export DIETPI_ASSUME_KERNEL_INSTALLED=1
	...
	#	OMV: https://dietpi.com/forum/t/1903
...
_EOF_
	...
	elif [[ $distro == '14.'* || $distro == 'forky/sid' ]]
	then
		G_DISTRO=9
		G_DISTRO_NAME='forky'
	else
	...
	Main()
	{
		...
		do
			...
			then
				...
				G_DIETPI-NOTIFY 1 'No IMAGE_CREATOR input. Aborting...\n'
				exit 1
			fi

			# Disallowed names
			local aDISALLOWED_NAMES=(
		...
		then
			...
			then
				...
				G_DIETPI-NOTIFY 1 'No PREIMAGE_INFO input. Aborting...\n'
				exit 1
			fi
		fi
		...
		local userland_arch=$(dpkg --print-architecture)
		case $G_HW_ARCH in
			1)
		...
		do
			...
			then
				...
				G_DIETPI-NOTIFY 1 "No valid hardware model input: HW_MODEL=\"$HW_MODEL\". Aborting...\n"
				exit 1
			fi
		done
...
\nWithout a partition table, the bootloader cannot be flashed, which can leave this system unbootable. It is however expected with some VPS providers, like Linode, which use external bootloaders. If this is the case, hit "Continue" to run the DietPi-Installer regardless.'
				then
					...
					then
						...
						G_DIETPI-NOTIFY 1 'Non-interactive call. Aborting...\n'
						exit 1
					fi
				fi
			else
		...
		# WiFi selection
		[[ $G_HW_MODEL =~ ^(20|75)$ ]] && WIFI_REQUIRED=0
		if [[ $WIFI_REQUIRED != [01] ]]
		then
			G_WHIP_MENU_ARRAY=(
			...
			then
				...
				G_DIETPI-NOTIFY 1 "No valid WiFi choice input: WIFI_REQUIRED=\"$WIFI_REQUIRED\". Aborting...\n"
				exit 1
			fi
		fi
		...
		DISTRO_LIST_ARRAY=(
			...
			'6' ': Bullseye (oldoldstable ancient release, NOT recommended)'
			'7' ': Bookworm (oldstable previous release, NOT recommended)'
			'8' ': Trixie (current stable release, recommended)'
			'9' ': Forky (testing, if you want to live on bleeding edge)'
		)

		# - Prevent downgrades and allow Bullseye for container images only (used for our automated tests) and SBCs which cannot run Bookworm due to ancient kernel version
		G_WHIP_MENU_ARRAY=()
		for ((i=0; i<${#DISTRO_LIST_ARRAY[@]}; i+=2))
		do
			(( ${DISTRO_LIST_ARRAY[$i]} < $G_DISTRO )) && continue
			(( ${DISTRO_LIST_ARRAY[$i]} < 7 && $G_HW_MODEL != 75 && $G_HW_MODEL != 61 && $G_HW_MODEL != 62 && $G_HW_MODEL != 70 )) && continue
		...
		do
			...
			then
				...
				G_DIETPI-NOTIFY 1 "No valid distro input: DISTRO_TARGET=\"$DISTRO_TARGET\". Aborting...\n"
				exit 1
			fi
		done
		...
		elif (( $DISTRO_TARGET == 8 ))
		then
			DISTRO_TARGET_NAME='trixie'
		else
			DISTRO_TARGET_NAME='forky'
		fi
...
setenv rootuuid "true"' /boot/boot.cmd
				# shellcheck disable=SC2016
				(( $G_HW_MODEL == 17 )) && G_EXEC sed --follow-symlinks -i '/^setenv overlay_error/a\
\n# petitboot workarounds\
...
setenv rootuuid "true"' /boot/boot.cmd
			...
			then
				...
				case $G_HW_MODEL in
					...
					43|55|56) G_CONFIG_INJECT 'overlay_prefix=' 'overlay_prefix=rockchip-rk3328 rockchip' /boot/dietpiEnv.txt;; # RK3328 mainline
			...
			# Allwinner 64-bit
			elif [[ $G_HW_MODEL =~ ^(40|44|45|57|65|67|83|88|89|96|99)$ ]]
			then
				G_EXEC sed --follow-symlinks -Ei '/^setenv (kernel|fdt)_addr_r/d' /boot/boot.cmd
				...
				case $G_HW_MODEL in
					...
					96) G_CONFIG_INJECT 'fdtfile=' 'fdtfile=allwinner/sun55i-t527-orangepi-4a.dtb' /boot/dietpiEnv.txt;;
					*) :;;
				esac
				# Overlay prefix
				case $G_HW_MODEL in
					...
					96) G_CONFIG_INJECT 'overlay_prefix=' 'overlay_prefix=sun55i-t527' /boot/dietpiEnv.txt;;
					*) :;;
				esac
				# Overlays
				case $G_HW_MODEL in
		...
		local apackages=()
		mapfile -t apackages < <(dpkg -l | grep -v '^ii ' | tail +6)
		...
		then
			...
			then
				G_EXEC curl -sSf 'https://archive.raspberrypi.com/debian/pool/main/r/raspberrypi-archive-keyring/raspberrypi-archive-keyring_2025.1+rpt1_all.deb' -o keyring.deb
				G_EXEC dpkg -i keyring.deb
				G_EXEC rm keyring.deb
				aPACKAGES_REQUIRED_INSTALL+=('raspberrypi-archive-keyring')
			fi
			# Block packages from RPi desktop which conflict with a native LXDE desktop:
			# - https://github.com/MichaIng/DietPi/issues/1558#issuecomment-691206284
			# - https://github.com/MichaIng/DietPi/issues/4687
			G_EXEC eval 'cat << '\''_EOF_'\'' > /etc/apt/preferences.d/dietpi-raspberrypi
...
_EOF_'
		...
		then
			...
			local apackages=("linux-image-$userland_arch")

			# VM: Enable systemd-logind by default for ACPI functionality, required for shutdown/reboot via virtualiser
			if (( $G_HW_MODEL == 20 ))
			then
				apackages+=('dbus')
			...
			if (( $G_HW_ARCH != 10 )) || [[ -d '/boot/efi' ]] || dpkg-query -s "grub-efi-$userland_arch" &> /dev/null
			then
				apackages+=("grub-efi-$userland_arch" "grub-efi-$userland_arch-signed" 'shim-signed')
...
_EOF_
			...
			then
				G_EXEC sed --follow-symlinks -i '/^setenv scriptaddr /a\
\
# Select Tablet Base Board device tree automatically if second USB-C port seems functional\
if usb info usb@fcd00000; then\
	echo "Second USB-C port seems functional, switching to Tablet Base Board default device tree"\
	setenv fdtfile "rockchip/rk3588s-orangepi-cm5-tablet.dtb"\
fi' /boot/boot.cmd
			fi
...
_EOF_
			...
			# Orange Pi Zero 3: Enable SPI1 OOTB
			elif (( $G_HW_MODEL == 83 ))
			then
				G_AGI device-tree-compiler
				G_EXEC mkdir -p /boot/overlay-user
				cat << '_EOF_' > /boot/overlay-user/dietpi-spi1_1.dts
/dts-v1/;
/plugin/;
/ {
...
_EOF_
				G_EXEC dtc -I dts -O dtb -o /boot/overlay-user/dietpi-spi1_1.{dtbo,dts}
				#G_CONFIG_INJECT 'user_overlays=' 'user_overlays=dietpi-spi1_1-otg' /boot/dietpiEnv.txt
				G_EXEC apt-mark auto device-tree-compiler
			fi
		...
		then
			...
			then
				...
				case $G_HW_MODEL in
					0)
						[[ $userland_arch == 'arm64' ]] || { G_EXEC dpkg --add-architecture arm64; G_AGUP; }
						case $G_HW_ARCH in
							1) akernel=('linux-image-rpi-v6' 'linux-image-rpi-v7' 'linux-image-rpi-v8');;
							*)
								if [[ $userland_arch == 'armhf' ]]
								then
									akernel=('linux-image-rpi-v7' 'linux-image-rpi-v8')
								else
									akernel=('linux-image-rpi-v8' 'linux-image-rpi-2712')
								fi
							;;
						esac
					;;
					1) akernel=('linux-image-rpi-v6');;
					2) akernel=('linux-image-rpi-v7');;
					4) akernel=('linux-image-rpi-v8');;
					5) akernel=('linux-image-rpi-2712');;
		...
		then
			...
			# Install testing kernel/U-Boot build if requested
			if (( $TEST_KERNEL || $TEST_UBOOT ))
			then
...
_EOF_
		...
		then
			...
			then
				...
				aPACKAGES_REQUIRED_INSTALL+=('firmware-realtek')			# Realtek Eth/WiFi
				# WiFi-only firmware
				if (( $WIFI_REQUIRED ))
				then
					...
					# - https://packages.debian.org/trixie/firmware-misc-nonfree
					if (( $G_DISTRO < 8 ))
					then
						...
						aPACKAGES_REQUIRED_INSTALL+=('firmware-carl9170')	# Qualcomm Atheros AR9170 WiFi
						aPACKAGES_REQUIRED_INSTALL+=('firmware-mediatek')	# MediaTek/Ralink WiFi
					fi
				fi
			fi
		fi
		...
		getent passwd openmediavault-webgui > /dev/null && userdel -f openmediavault-webgui # OMV
		getent passwd admin > /dev/null && userdel -f admin # OMV
		getent passwd fa > /dev/null && userdel -f fa # OMV
		getent passwd colord > /dev/null && userdel -f colord # OMV
		getent passwd saned > /dev/null && userdel -f saned # OMV
		getent passwd orangepi > /dev/null && userdel -f saned # Orange Pi images
		getent group openmediavault-config > /dev/null && groupdel openmediavault-config # OMV
		getent group openmediavault-engined > /dev/null && groupdel openmediavault-engined # OMV
		getent group openmediavault-webgui > /dev/null && groupdel openmediavault-webgui # OMV
...
_EOF_'
		...
		# Amlogic
		elif [[ $kernel == 'meson' || $kernel == 'meson64' ]]
		then
			/boot/dietpi/func/dietpi-set_hardware serialconsole enable ttyAML0

		# Allwinner, StarFive, ROCK Pi S
		elif [[ $kernel == 'sunxi' || $kernel == 'sunxi64' || $G_HW_MODEL =~ ^(73|81|84)$ ]]
		then
		...
		G_EXEC ln -snf /usr/share/zoneinfo/UTC /etc/localtime
		if (( $G_DISTRO < 7 ))
		then
			G_EXEC eval 'echo '\''ETC'\'' > /etc/timezone'
		else
			[[ -e '/etc/timezone' ]] && G_EXEC rm /etc/timezone # deprecated since Bookworm
		fi
...
_EOF_
		...
		# Update initramfs
		command -v 'update-initramfs' &> /dev/null && G_EXEC_OUTPUT=1 G_EXEC update-initramfs -u

		# GRUB install and config
		if dpkg-query -s 'grub2-common' &> /dev/null
		then
			# UEFI
			if [[ -d '/boot/efi' ]] && dpkg-query -s "grub-efi-$userland_arch" &> /dev/null
			then
				...
				case $G_HW_ARCH in
					3) local grub_arch='aa64';;
					11) local grub_arch='riscv64';;
					*) local grub_arch='x64';;
				esac
				shopt -s nocaseglob
				local efi_fallback=()
				# shellcheck disable=SC2043,SC2231
				for i in /boot/efi/EFI/boot/boot$grub_arch.efi*
				do
					[[ -e $i ]] && break
					efi_fallback=('--force-extra-removable')
					G_EXEC eval "debconf-set-selections <<< 'grub-efi-$userland_arch grub2/force_efi_extra_removable boolean true'"
				done
				shopt -u nocaseglob
				G_EXEC_DESC='Installing GRUB for UEFI' G_EXEC_OUTPUT=1 G_EXEC grub-install --recheck --target="${userland_arch/amd64/x86_64}-efi" --efi-directory=/boot/efi "${efi_fallback[@]}" --uefi-secure-boot
		...
		find /usr -type d -name '__pycache__' -exec rm -Rv {} +
